  ...
  grid.innerHTML = products
    .map((p) => {
      ...
      if (offerValid) {
        ...
        `;
        offerBadgeHtml = `<span class="shop-offer-tag" style="position: absolute; top: 8px; right: 8px; background: var(--error, #ff6b6b); color: #fff; font-size: 0.72rem; font-weight: 600; padding: 3px 8px; border-radius: 4px; z-index: 5;">${offerName ? `${escapeHtml(offerName)} · ` : ""}${pct}% OFF</span>`;
      }
      ...
      return `
        <a class="product-card" href="/product-detail.html?id=${encodeURIComponent(productId)}" style="position: relative;">
          ...
          <div class="img-box">
            ${firstImage
              ? `<img src="${firstImage.startsWith("http") ? firstImage : `/uploads/${firstImage}`}" alt="${escapeHtml(p.name)}" />`
              : "No image"}
          </div>
          <div class="info">
            <div class="cat-label">${escapeHtml(categoryName)}</div>
